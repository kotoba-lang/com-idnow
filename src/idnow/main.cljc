(ns idnow.main
  "Kotodama WASM entrypoint for the idnow clean-room actor (L4) — Clojure port.
  ...
  facts (`idnow.<Entity>/<field>`); `*store*` is the in-memory materialization
...
(defn emit-facts
  "EAVT facts for one record: {\"idnow.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "idnow-compat" :tier tier :entities entities} 200])
